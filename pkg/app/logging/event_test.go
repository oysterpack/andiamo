...
func TestEvent_Equals(t *testing.T) {
	...
	event1 := logging.NewEvent("foo", zerolog.InfoLevel)
	event2 := logging.NewEvent("foo", zerolog.WarnLevel)
	event3 := logging.NewEvent("foo", zerolog.InfoLevel, A)
	event4 := logging.NewEvent("foo", zerolog.InfoLevel, A, B)
	event5 := logging.NewEvent("foo", zerolog.InfoLevel, C)
	event6 := logging.NewEvent("foo", zerolog.InfoLevel, B, A, B, A)
	event7 := logging.NewEvent("foo", zerolog.InfoLevel, A, C)
...
func TestEvent_Log(t *testing.T) {
	...
	var (
		...
		FooEvent = logging.NewEvent("foo", zerolog.WarnLevel, DGraphTag, DataTag, DataTag, "DATA")
